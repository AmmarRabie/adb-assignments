...
//here we create a sample test to read and write to our database file
  ...
  //1. Create Database file or Open it if it already exists, check readfile.cpp
   char* dbPath = "db/chaining";
   if (argc >= 3) {
     dbPath = argv[1];
   }

   filehandle = createFile(FILESIZE, dbPath);
   string writePath = string("tests/res/") + "c_" + string(basename(argv[1]));
  ...
  testfile >> operation;
   while (!testfile.eof()){
     ...
     {
     case 'V':
     resultFile << "* view file operation" << endl;
  ...
  //3. Add some data in the table
   ...
   insert(6, 70);         //1  h(key) = 1.0, 1.1
   insert(42, 80);        //3  h(key) = 2.0
   insert(4, 25);         //4  h(key) = 4.0
   insert(12, 44);        //5  h(key) = 2.0, 2.1
   insert(14, 32);        //6  h(key) = 4.0, 4.1
   insert(17, 11);        //7  h(key) = 2.0, 2.1, OF0.0
   insert(13, 78);        //8  h(key) = 3.0
   insert(37, 97);        //9  h(key) = (2) OF0.0, OF0.1
   insert(11, 34);        //10 h(key) = 1.0, 1.1, OF0.0, OF0.1, OF1.0
   insert(22, 730);       //11 h(key) = (2) OF0.0, OF0.1, OF1.0, OF1.1
   insert(46, 840);       //12 h(key) = (1) OF0.0, OF0.1, OF1.0, OF1.1, OF2.0
   insert(9, 83);         //13 h(key) = 4.0, 4.1, OF0.0, OF0.1, OF1.0, OF1.1, OF2.0, OF2.1
   insert(21, 424);       //14 h(key) = (1) OF0.0, OF0.1, OF1.0, OF1.1, OF2.0, OF2.1, OF3.0
   insert(41, 115);       //15 h(key) = (1) OF0.0, OF0.1, OF1.0, OF1.1, OF2.0, OF2.1, OF3.0, OF3.1
   insert(71, 47);        //16 h(key) = (1) OF0.0, OF0.1, OF1.0, OF1.1, OF2.0, OF2.1, OF3.0, OF3.1, OF4.0
   insert(31, 92);        //17 h(key) = (1) OF0.0, OF0.1, OF1.0, OF1.1, OF2.0, OF2.1, OF3.0, OF3.1, OF4.0, OF4.1
   insert(73, 45);        //18 h(key) = 3.0, 3.1
