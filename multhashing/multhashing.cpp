...
int insertItem(int fd, DataItem item){
   //Definitions
	...
	{ 	 // perror("some error occurred in pread");
    	...
    	//I found the first free space (write on it)
				ssize_t pwriteResult = pwrite(fd, &item, sizeof(DataItem), Offset);
				// pritnf(pwriteResult)
    			return count;

    } else { // there is a results returned and the data is valid, don't overwrite, advance offset
		if (count == RECORDSPERBUCKET) Offset = hashCode(item.key, true) * BUCKETSIZE;
		else
		{
...
{
	DataItem* item = (struct DataItem *) malloc(sizeof(struct DataItem));
	item->key = key;
	int diff = 0;
	int Offset= searchItem(fd, item, &diff);
	if(Offset >=0 )
	{
		return deleteOffset(fd, Offset);
	}
	return -1;
}
